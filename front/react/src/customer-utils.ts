...
const parsePicture = ({ sprites }: { sprites: Sprites }) => sprites.front_default;
...
export const parseCustomersList =
    ({ results }: CustomerListResponse): Customer[] => results;
...
export const parseCustomerData = ({ name, id, height, weight, ...customer }: CustomerResponse): CustomerData => ({
    id,
    name,
    height,
    weight,
